...
/* Compress + resize image to max 900px, JPEG 75% quality.
   Uses canvas (works on all mobile browsers).
   Falls back to FileReader if canvas fails. */
...
async function previewImg(input, prevId, phId) {
  ...
  if (!prev) return;
  try {
    var b64 = await compressImage(file, 900, 0.75);
    if (!b64) { showToast('\u26a0 \u0e44\u0e21\u0e48\u0e2a\u0e32\u0e21\u0e32\u0e23\u0e16\u0e42\u0e2b\u0e25\u0e14\u0e23\u0e39\u0e1b\u0e44\u0e14\u0e49'); return; }
    prev.src = b64;
    ...
    console.warn('previewImg error:', e);
    showToast('\u26a0 \u0e40\u0e01\u0e34\u0e14\u0e02\u0e49\u0e2d\u0e1c\u0e34\u0e14\u0e1e\u0e25\u0e32\u0e14\u0e43\u0e19\u0e01\u0e32\u0e23\u0e42\u0e2b\u0e25\u0e14\u0e23\u0e39\u0e1b');
  }
}
...
function updateSwipeClock() {
  var el = document.getElementById('swipe-time-display');
  if (!el) return;
  var now = new Date();
